...
public class Ranks {
	...
	public void setRank(String player, String buyClass, boolean buy){
		if(plugin.getHousesConfig().getConfig().getBoolean("use-class-ranks")){
			try {
				...
				if(rank != null){
					if(buy){
						...
						if(rs.next()){
							...
							removeRanks(world, groups, player);
							Houses.permission.playerAddGroup(world, player, plugin.getHousesConfig().getConfig().getString("homeless"));
